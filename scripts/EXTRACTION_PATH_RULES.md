...
```
<WORKSPACE_ROOT>/
├── blockchain-nft-interactive/          # Main project
│   ├── scripts/                         # Extraction scripts
│   ├── src/                             # Source code
│   └── ...
└── grant-repositories/                  # Extracted grant repos (external to project)
...
- Reference folders:
  - `<WORKSPACE_ROOT>/blockchain-ai-ml-references`
  - `<WORKSPACE_ROOT>/reference_repos`
...
1. **ALWAYS run extraction scripts from project root (`blockchain-nft-interactive`):**
   ```bash
   cd "<WORKSPACE_ROOT>/blockchain-nft-interactive"
...
3. **Use explicit paths for output directories:**
   ```bash
   # CORRECT
   mkdir -p "<WORKSPACE_ROOT>/grant-repositories/near-creative-engine"
...
- [ ] Output directory points to external `grant-repositories/`
...
2. **Create directories with error checking:**
   ```bash
   GRANT_DIR="<WORKSPACE_ROOT>/grant-repositories/near-creative-engine"
