...
# Enhanced NEAR Testnet Deployment Script - FIXED VERSION
# This script handles real testnet deployment with proper error handling
# SKIPS BUILD STEP - uses pre-compiled WASM

set -e

echo "🚀 Starting Real NEAR Testnet Deployment (FIXED VERSION)..."
...
if ! command -v near &> /dev/null; then
    echo -e "${YELLOW}⚠️  near-cli not found, installing...${NC}"
...
near create-account "$SUBACCOUNT_NAME.$NEAR_ACCOUNT_ID" \
...
near deploy "$SUBACCOUNT_NAME.$NEAR_ACCOUNT_ID" \
    "$CONTRACT_WASM" \
...
near view "$SUBACCOUNT_NAME.$NEAR_ACCOUNT_ID" nft_metadata || {
...
near call "$SUBACCOUNT_NAME.$NEAR_ACCOUNT_ID" mint_soulbound \
    '{
        "emotion_data": {
...
near view "$SUBACCOUNT_NAME.$NEAR_ACCOUNT_ID" nft_tokens_for_owner \
