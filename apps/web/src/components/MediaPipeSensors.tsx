...
import { UnifiedAIMLPipeline } from '../utils/unified-ai-ml-integration';
...
const MediaPipeSensors: React.FC<MediaPipeSensorsProps> = ({ className, onMetrics }) => {
  ...
  const aiPipeline = useRef<UnifiedAIMLPipeline | null>(null);

  // Moved hooks from inside useEffect to top level
  const [audioFeatures, setAudioFeatures] = useState<{ pitch: number; energy: number; emotion: string } | null>(null);
  const [leapMotionFeatures, setLeapMotionFeatures] = useState<{ gesture: string; handPosition: { x: number; y: number; z: number } } | null>(null);
  ...
  useEffect(() => {
    ...
    const init = async () => {
      try {
        ...
        aiPipeline.current = new UnifiedAIMLPipeline();
        await aiPipeline.current.initialize();

        // Dynamically load MediaPipe modules
        let MediaPipeCamera, MediaPipeHands, MediaPipeFaceMesh, MediaPipePose;
        try {
          const [cameraModule, handsModule, faceMeshModule, poseModule] = await Promise.all([
            import('@mediapipe/camera_utils').catch(() => null),
            import('@mediapipe/hands').catch(() => null),
            import('@mediapipe/face_mesh').catch(() => null),
            import('@mediapipe/pose').catch(() => null)
          ]);

          MediaPipeCamera = cameraModule?.Camera;
          MediaPipeHands = handsModule?.Hands;
          MediaPipeFaceMesh = faceMeshModule?.FaceMesh;
          MediaPipePose = poseModule?.Pose;

          if (!MediaPipeCamera || !MediaPipeHands || !MediaPipeFaceMesh || !MediaPipePose) {
            console.warn('⚠️ MediaPipe modules not available, sensor tracking disabled');
            setStatus('MediaPipe not available');
            return;
          }
        } catch (error) {
          console.warn('⚠️ Failed to load MediaPipe modules:', error);
          setStatus('MediaPipe not available');
          return;
        }

        if (aiPipeline.current.wasmBridge.voiceIntegration) {
          await aiPipeline.current.wasmBridge.voiceIntegration.initialize();
          const processAudio = async () => {
            if (!running || !aiPipeline.current?.wasmBridge.voiceIntegration) return;
            const features = aiPipeline.current.wasmBridge.voiceIntegration.get_audio_features();
            if (features) {
              setAudioFeatures({
                pitch: features.pitch,
                energy: features.energy,
                emotion: features.emotion_from_voice,
              });
            }
        ...
        if (aiPipeline.current.wasmBridge.leapMotionIntegration) {
          const leap = new aiPipeline.current.wasmBridge.leapMotionIntegration.LeapMotionIntegration();
          await leap.connect();
          const processLeapMotion = async () => {
            if (!running || !aiPipeline.current?.wasmBridge.leapMotionIntegration) return;
            const frameData = leap.get_frame_data();
            if (frameData) {
              const parsedData = JSON.parse(frameData);
              if (parsedData.hands && parsedData.hands.length > 0) {
                const hand = parsedData.hands[0];
                setLeapMotionFeatures({
                  gesture: hand.gestures && hand.gestures.length > 0 ? hand.gestures[0].type : 'none',
                  handPosition: { x: hand.palmPosition[0], y: hand.palmPosition[1], z: hand.palmPosition[2] },
                });
              }
            }
            requestAnimationFrame(processLeapMotion);
          };
          processLeapMotion();
        }

        hands = new MediaPipeHands({ locateFile: (file: string) => `https://cdn.jsdelivr.net/npm/@mediapipe/hands/${file}` });
        ...
        faceMesh = new MediaPipeFaceMesh({ locateFile: (file: string) => `https://cdn.jsdelivr.net/npm/@mediapipe/face_mesh/${file}` });
        ...
        pose = new MediaPipePose({ locateFile: (file: string) => `https://cdn.jsdelivr.net/npm/@mediapipe/pose/${file}` });
        ...
        pose.onResults((results: any) => {
          ...
          if (results.poseLandmarks && results.poseLandmarks.length > 0) {
            ...
            if (lastPose && lastPose.length === current.length) {
              ...
              poseStability = Math.max(0, 1 - avgDelta * 10);
            }
            lastPose = current;
          }
          setCounts(prev => {
  ...
  }, [audioFeatures, leapMotionFeatures, onMetrics]); // Added missing dependencies
