...
const FilecoinStorageIntegration: React.FC<FilecoinStorageIntegrationProps> = ({
  ...
  const [provider, setProvider] = useState<'web3' | 'nft'>('web3');
  const [isConnected, setIsConnected] = useState(false);
  const progressIntervalRef = useRef<NodeJS.Timeout | null>(null);

  const connectStorage = useCallback(async () => {
    if (!apiKey.trim()) {
      onError?.(`Please enter an API key for ${provider === 'web3' ? 'Web3.Storage' : 'NFT.Storage'}`);
      return;
    }
  ...
  return (
    <div className="bg-gray-800 rounded-lg p-6">
        ...
        <div className="space-y-4">
          <div className="flex gap-2 mb-4">
            <button
              ...
              className={`flex-1 py-2 px-3 rounded-md text-xs font-medium transition-all ${provider === 'web3' ? 'bg-blue-600 text-white' : 'bg-gray-700 text-gray-400'}`}
            >
              Web3.Storage
            </button>
            <button
              onClick={() => setProvider('nft')}
              className={`flex-1 py-2 px-3 rounded-md text-xs font-medium transition-all ${provider === 'nft' ? 'bg-teal-600 text-white' : 'bg-gray-700 text-gray-400'}`}
            >
              NFT.Storage
            </button>
          </div>
          ...
          <div>
            <label className="block text-sm font-medium text-gray-300 mb-2">
              {provider === 'web3' ? 'Web3.Storage' : 'NFT.Storage'} API Key
            </label>
            <input
              type="password"
              value={apiKey}
              onChange={(e) => setApiKey(e.target.value)}
              placeholder={`Enter your ${provider === 'web3' ? 'Web3.Storage' : 'NFT.Storage'} API key`}
      ...
      <div className="mt-6 pt-6 border-t border-gray-700">
        ...
        <HmplFeed endpoint="/hmpl/storage-health" />
      </div>
    </div>
  );
};
