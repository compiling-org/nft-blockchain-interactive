...
import { HMPLTemplateRenderer } from './HMPLTemplateRenderer';
...
export const ComprehensiveNEARCreativeEngine: React.FC<NEARCreativeEngineProps> = ({ className = '' }) => {
    ...
    try {
      ...
      if (account) {
        const contract = new Contract(account, CONTRACT_ID, {
          viewMethods: ['get_metadata', 'get_interaction_history', 'get_emotional_history', 'total_supply'],
          changeMethods: ['mint_nft', 'record_interaction', 'record_emotional_interaction']
        } as any);
  ...
  const generateCreativeAsset = async (type: CreativeAsset['type']) => {
    ...
    try {
      ...
      // Record interaction on chain using wallet service with packing
      if (walletService) {
        await walletService.recordEmotionalInteraction(
          asset.id,
          asset.emotionalState
        );
      } else {
        // Fallback or error
        console.warn('Wallet service not available for recording');
      }
    ...
    <div className="space-y-6">
      <div className="grid grid-cols-1 md:grid-cols-3 gap-6">
        <Card className="bg-gray-800 border-gray-700">
          <CardHeader>
            <CardTitle className="text-blue-400">Wallet Status</CardTitle>
          </CardHeader>
        ...
        <Card className="bg-gray-800 border-gray-700">
          <CardHeader>
            <CardTitle className="text-green-400">Emotional State</CardTitle>
          </CardHeader>
          <CardContent className="space-y-2">
        ...
        <CardContent>
          <div className="grid grid-cols-2 md:grid-cols-4 gap-4">
            <Button
              ...
              isDisabled={!isConnected || isGenerating}
      ...
      <div className="mt-6">
        <HMPLTemplateRenderer emotionalState={emotionalState} />
      </div>
    </div>
  );
