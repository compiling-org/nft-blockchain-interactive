...
export class HybridAIManager {
  ...
  async startAudioAnalysis(): Promise<void> {
  ...
  getLiveAudioData(): number[] {
    if (!this.analyser || !this.audioDataArray) return this.generateSyntheticAudio();
    this.analyser.getByteFrequencyData(this.audioDataArray);
    return Array.from(this.audioDataArray).map(v => v / 255);
  }
  ...
  async detectEmotion(eegData: number[], audioData: number[], sensorFeatures?: any): Promise<EmotionResult> {
    if (!this.isInitialized || !this.emotionModel || !this.eegModel || !this.audioModel) {
      // Fallback to enhanced synthetic emotion detection if features are provided
      if (sensorFeatures) {
        return this.processSensorFeatures(sensorFeatures);
      }
  ...
  private processSensorFeatures(features: any): EmotionResult {
    // Real mapping of MediaPipe/LeapMotion features to VAD
    const { faceVariance, handOpenness, poseStability, confidence } = features;

    // Valence is highly correlated with facial variance (expressions)
    const valence = Math.max(0, Math.min(1, 0.5 + (faceVariance - 0.1) * 2));

    // Arousal is tied to hand openness and activity levels
    const arousal = Math.max(0, Math.min(1, 0.3 + handOpenness * 1.5));

    // Dominance is tied to pose stability and confidence
    const dominance = Math.max(0, Math.min(1, 0.4 + poseStability * 0.5 + confidence * 0.2));
    ...
    return {
      ...
      stress: (1 - valence) * 0.5 + arousal * 0.5,
      confidence: 0.9
    };
  }
