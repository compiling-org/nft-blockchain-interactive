...
// Custom Progress component since shadcn one is missing
...
const PolkadotSoulboundIdentity: React.FC<PolkadotSoulboundIdentityProps> = ({
  ...
  const [verificationType, setVerificationType] = useState<'Biometric' | 'Social' | 'Government' | 'Community'>('Biometric');
  const [verificationPayload, setVerificationPayload] = useState('');
  const [verifyLoading, setVerifyLoading] = useState(false);
  const [verifyStatus, setVerifyStatus] = useState<string>('');
  const [refreshing, setRefreshing] = useState(false);
  const [reputationScore, setReputationScore] = useState(70);
  ...
  const calculateReputation = useCallback((v: number, a: number, d: number) => {
    // RS = (V * 0.4) + (A * 0.3) + (D * 0.3) * 100
    const score = Math.round(((v * 0.4) + (a * 0.3) + (d * 0.3)) * 100);
  ...
  const handleSensorMetrics = useCallback(async (metrics: any) => {
    ...
    if (ai) {
      ...
      calculateReputation(newEmotion.valence, newEmotion.arousal, newEmotion.dominance);
    }
  }, [calculateReputation]);

  const handleRequestVerification = async () => {
    if (!client || !keypair || !identity || !verificationPayload) return;
    try {
      setVerifyLoading(true);
      setVerifyStatus('');
      const data = new TextEncoder().encode(verificationPayload);
      const txHash = await client.requestVerification(
        keypair,
        identity.identity_id,
        verificationType,
        data
      );
      setVerifyStatus(txHash);
      const updated = await client.getIdentity(identity.identity_id);
      setIdentity(updated);
  ...
  const handleRefreshIdentity = async () => {
    ...
    try {
      ...
      setError(e instanceof Error ? e.message : 'Failed to refresh identity');
    } finally {
      setRefreshing(false);
    }
  };
  ...
  return (
    <div className="min-h-screen bg-gradient-to-br from-pink-900 via-red-900 to-gray-900 p-6">
      <div className="max-w-4xl mx-auto">
        <div className="text-center mb-8">
          <h1 className="text-4xl font-bold text-white mb-2">
            Polkadot Soulbound Identity
          </h1>
          <div className="inline-flex items-center gap-2 px-3 py-1 rounded-full bg-black/40 border border-pink-500/40 text-pink-200 text-sm">
            <Badge variant="outline" className="border-pink-500/50 text-pink-300">Westend Testnet</Badge>
          </div>
          <p className="text-pink-200 mt-3">
            Create a non-transferable identity with biometric verification and emotional state
          </p>
        </div>
        ...
        <div className="bg-black bg-opacity-30 rounded-lg p-4 mb-6">
          <div className="flex items-center justify-between">
            ...
            >
              {keypair ? 'Wallet Connected' : 'Connect Polkadot Wallet'}
            </button>
          </div>
        </div>
        ...
        <div className="bg-black bg-opacity-30 rounded-lg p-4 mb-6">
          <div className="grid grid-cols-1 md:grid-cols-3 gap-3">
            <button
              onClick={() => window.open('https://faucet.polkadot.io/westend', '_blank')}
              className="bg-pink-600 hover:bg-pink-700 text-white px-4 py-2 rounded-lg transition-colors"
            >
              Open Westend Faucet
            </button>
            <button
              onClick={() => window.open('https://faucet.polkadot.io/rococo', '_blank')}
              className="bg-pink-600 hover:bg-pink-700 text-white px-4 py-2 rounded-lg transition-colors"
            >
              Open Rococo Faucet
            </button>
            <button
              onClick={() => window.open('https://polkadot.js.org/apps', '_blank')}
            ...
            <div className="space-y-4">
              <div>
                ...
                <input
                  ...
                  className="w-full bg-gray-800 border border-gray-600 rounded-lg px-3 py-2 text-white focus:outline-none focus:ring-2 focus:ring-pink-500"
              ...
              <div className="space-y-3">
                ...
                <div>
                  <label className="block text-pink-200 text-sm font-medium mb-1">
              ...
              <div className="mt-6 mb-6">
                <MediaPipeSensors onMetrics={handleSensorMetrics} />
              </div>

              <button
                onClick={handleCreateIdentity}
                disabled={loading || !keypair || !identityName || !metadataUri}
                className="w-full bg-pink-600 hover:bg-pink-700 disabled:bg-gray-600 disabled:cursor-not-allowed text-white font-semibold py-3 px-4 rounded-lg transition-colors"
              >
          ...
          <div className="bg-black bg-opacity-30 rounded-lg p-6">
            <div className="flex items-center justify-between mb-4">
              ...
              {identity && (
                <button
                  ...
                  disabled={refreshing}
                  className="bg-gray-700 hover:bg-gray-600 disabled:bg-gray-600 disabled:cursor-not-allowed text-white px-3 py-2 rounded-lg text-sm transition-colors"
                >
                  {refreshing ? 'Refreshing...' : 'Refresh Identity'}
                </button>
              )}
            </div>
            ...
            {identity ? (
              <div className="space-y-4">
                ...
                <div className="bg-gray-800 rounded-lg p-4">
                  <h4 className="text-md font-medium text-white mb-2">Reputation</h4>
                  <div className="text-xs text-gray-400 mb-2">Score: {identity.reputation_score}/100</div>
                  <CustomProgress value={identity.reputation_score} />
                </div>
                ...
                <div className="bg-gray-800 rounded-lg p-4">
                  ...
                  <a
                    ...
                    className="text-pink-400 hover:text-pink-300 text-sm break-all"
                  >
                ...
                <div className="bg-gray-800 rounded-lg p-4">
                  <h4 className="text-md font-medium text-white mb-3">Request Verification</h4>
                  <div className="grid grid-cols-1 md:grid-cols-2 gap-3">
                    <div>
                      <label className="block text-pink-200 text-sm font-medium mb-2">Type</label>
                      <select
                        value={verificationType}
                        onChange={(e) => setVerificationType(e.target.value as any)}
                        className="w-full bg-gray-900 border border-gray-700 rounded-lg px-3 py-2 text-white focus:outline-none focus:ring-2 focus:ring-pink-500"
                      >
                    ...
                    <div>
                      <label className="block text-pink-200 text-sm font-medium mb-2">Payload</label>
                      <input
                        type="text"
                        value={verificationPayload}
                        onChange={(e) => setVerificationPayload(e.target.value)}
                        className="w-full bg-gray-900 border border-gray-700 rounded-lg px-3 py-2 text-white focus:outline-none focus:ring-2 focus:ring-pink-500"
                        placeholder="Evidence or reference"
                      />
                    </div>
                  </div>
                  <div className="mt-3">
                    <button
                      ...
                      disabled={verifyLoading || !keypair || !verificationPayload}
                      className="bg-pink-600 hover:bg-pink-700 disabled:bg-gray-600 disabled:cursor-not-allowed text-white font-semibold py-2 px-4 rounded-lg transition-colors"
                    >
                      {verifyLoading ? 'Requesting...' : 'Request Verification'}
                    </button>
                    {verifyStatus && (
                      <div className="mt-2 text-xs text-gray-300 break-all">
                        InBlock: {verifyStatus}
                      </div>
                    )}
                  </div>
                </div>
        ...
        <div className="bg-black bg-opacity-30 rounded-lg p-6 mt-6">
          ...
          <div className="grid grid-cols-1 md:grid-cols-3 gap-4 text-sm text-pink-200">
            <div>
            ...
            <div>
              <h4 className="font-medium text-white mb-2">📈 Reputation & Verification</h4>
              <p>Request verifications and build reputation tied to your identity across Polkadot ecosystems.</p>
            </div>
          </div>
        </div>
      </div>
