...
import { EnhancedBiometricNFTClient, createEnhancedAnchorProvider } from '../utils/solana-client-enhanced';
...
const SolanaEmotionalNFT: React.FC = () => {
  ...
  const [storageProvider, setStorageProvider] = useState<'nft' | 'web3'>('nft');
  ...
  useEffect(() => {
    if (wallet.connected && wallet.publicKey) {
      const initClient = async () => {
        try {
          toast.loading('Initializing Enhanced NFT client & AI Models...', {
            duration: 4000,
            position: 'top-center',
          });

          const provider = createEnhancedAnchorProvider(connection, wallet as any);
          
          // Initialize enhanced client with cross-chain configuration
          const client = new EnhancedBiometricNFTClient(
            connection, 
            provider,
            {
              near: {
          ...
          await client.initializeModels();
          
          setNftClient(client);

          console.log('Enhanced NFT client initialized with wallet:', wallet.publicKey.toString());

          toast.success('🎉 Enhanced Client & AI Models Ready!', {
            duration: 3000,
            position: 'top-center',
            description: 'Ready to mint enhanced biometric NFTs',
          });
  ...
  const startBiometricCapture = async () => {
    ...
    try {
      ...
      if (aiManagerRef.current) {
        await aiManagerRef.current.startAudioAnalysis();
      }
      ...
      const process = async () => {
        ...
        if (v && c) {
          ...
          if (ctx) {
            ...
            // Fetch real audio and compute emotion
            const ai = aiManagerRef.current;
            if (ai) {
              const audioData = ai.getLiveAudioData();
              const eegData = ai.generateSyntheticEEG();
              const result = await ai.detectEmotion(eegData, audioData, sensorFeatures);
              ...
              setMicLevel(audioData.reduce((a, b) => a + b, 0) / audioData.length);
            }
          }
        }
  ...
  const handleMediaPipeMetrics = useCallback(async (metrics: { hands: number; faces: number; poses: number; features?: { faceVariance: number; handOpenness: number; poseStability: number; confidence: number } }) => {
    ...
    // Use Real AI Manager for inference if models are ready
    const ai = aiManagerRef.current;
    if (ai) {
      const audioData = ai.getLiveAudioData();
      const eegData = ai.generateSyntheticEEG();
      ...
      setEmotionData(prev => ({
        ...prev,
        valence: (prev.valence * 0.5) + (result.valence * 0.5),
        arousal: (prev.arousal * 0.5) + (result.arousal * 0.5),
        dominance: (prev.dominance * 0.5) + (result.dominance * 0.5),
      }));
    }
  }, []);
  ...
  const captureBiometricData = async () => {
    ...
    try {
      ...
      toast('⚠️ Using fallback biometric hash', {
        duration: 4000,
        position: 'top-center',
  ...
  const mintEmotionalNFT = async () => {
    ...
    setIsMinting(true);
    try {
      // Calculate base quality score locally for metadata
      const valenceScore = Math.abs(emotionData.valence - 0.5) * 2;
      const baseQuality = (valenceScore * 0.4 + emotionData.arousal * 0.3 + emotionData.dominance * 0.3);
      ...
      const metadata: NFTMetadata = {
        name: `Enhanced Emotional NFT #${Date.now()}`,
        symbol: 'ENHANCED-EMO',
        description: `AI-enhanced biometric NFT. Valence: ${emotionData.valence.toFixed(2)}, Arousal: ${emotionData.arousal.toFixed(2)}`,
        image: imageCid ? `https://ipfs.io/ipfs/${imageCid}` : aiGeneratedArt,
        animation_url: sessionCid ? `https://ipfs.io/ipfs/${sessionCid}` : (imageCid ? `https://ipfs.io/ipfs/${imageCid}` : aiGeneratedArt),
        attributes: [
          ...
          { trait_type: 'Quality Score', value: qualityScore },
          { trait_type: 'AI Model', value: 'Enhanced-v2.1' },
      ...
      console.log('🎨 Creating enhanced biometric NFT with metadata:', metadata);

      let cid = '';
      if (storageToken) {
          // Use enhanced client's metadata upload if available
          try {
            if ((nftClient as any).uploadEnhancedMetadata) {
               cid = await (nftClient as any).uploadEnhancedMetadata(metadata);
            } else {
               // Fallback
               const json = JSON.stringify(metadata);
               cid = await uploadToIPFS(json);
            }
            setMetadataCid(cid);
          } catch (e) {
            console.warn('Metadata upload failed, proceeding with mint', e);
          }
      }

      // Construct biometric input for enhanced client
      const biometricInput = {
        eeg: {
          alpha: emotionData.valence,
          beta: emotionData.arousal,
          theta: emotionData.dominance,
          gamma: 0.5,
          delta: 0.2
        },
        attention: Math.round(emotionData.valence * 100),
        meditation: Math.round((1 - emotionData.arousal) * 100),
        gesture: {
          confidence: sensorFeatures?.confidence || 0.8,
          type: sensorCounts.hands > 0 ? 'active_interaction' : 'passive_observation'
        },
        audio: {
          confidence: 0.9,
          intensity: micLevel,
        ...
        signalQuality: (sensorFeatures?.confidence || 0.8) * 0.9,
        timestamp: Date.now()
      };

      // On-chain program mint (createEnhancedNFT)
      const result = await nftClient.createEnhancedNFT(
        wallet.publicKey,
        biometricInput,
        metadata
      );
      ...
      try {
        if ((nftClient as any).sendMemoWithWallet) {
          const memoSig = await (nftClient as any).sendMemoWithWallet(
            wallet,
            `ENHANCED_MINT:${emotionData.biometricHash}:${result.qualityScore}:${cid || 'no_cid'}`
          );
      ...
      toast.success('🎨 Enhanced Biometric NFT minted!', {
        duration: 4000,
        position: 'top-center',
        description: `Tx: ${result.transactionSignature.slice(0, 8)}...`,
      });

    } catch (error) {
      console.error('❌ Failed to mint NFT:', error);
      let errorMessage = 'Failed to mint Enhanced NFT. ';
        ...
        <div className="grid grid-cols-1 lg:grid-cols-2 gap-8">
          ...
          <div className="bg-gray-800 rounded-lg p-6">
            ...
            <div className="mt-4 grid grid-cols-1 gap-2">
              ...
              <div className="flex items-center gap-2">
                <span className="text-xs text-gray-400">Storage Provider:</span>
                <select
                  value={storageProvider}
                  onChange={(e) => setStorageProvider(e.target.value as 'nft' | 'web3')}
