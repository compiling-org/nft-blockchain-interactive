...
export const AIMLBlockchainIntegration: React.FC = () => {
  ...
  const [selectedGpuDevice, setSelectedGpuDevice] = useState<string>('Auto');
  ...
  const executeIntegration = useCallback(async (): Promise<void> => {
    ...
    try {
      ...
      const gpuReady = await setupGPUAcceleration(selectedGpuDevice);
  ...
  const selectGpuDevice = useCallback(async (device: string): Promise<boolean> => {
    console.log(`Attempting to select GPU device: ${device}`);
    await new Promise(resolve => setTimeout(resolve, 50));
    setSelectedGpuDevice(device);
    return true;
  }, []);

  const setupGPUAcceleration = async (device: string): Promise<boolean> => {
    // Based on Candle multi-backend device selection
    // candle/src/device.rs:145-189
    console.log(`Setting up GPU acceleration for device: ${device}...`);
    
    // Simulate device selection based on selectedGpuDevice
          ...
          <div className="bg-gray-800 rounded-lg p-6 border border-gray-700">
            ...
            <div className="space-y-4">
              <div className="flex items-center space-x-2">
                <label htmlFor="gpu-device-select" className="text-gray-300">Select GPU Device:</label>
                <select
                  ...
                  onChange={(e) => selectGpuDevice(e.target.value)}
                  disabled={isProcessing}
                >
                  <option value="Auto">Auto (Candle)</option>
