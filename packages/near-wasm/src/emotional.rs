...
#[serde(crate = "near_sdk::serde")]
#[serde(transparent)]
pub struct CompactEmotionalState(pub u64);

impl CompactEmotionalState {
    /// Create a new CompactEmotionalState from float values
    /// Valence: -1.0 to 1.0 (10 bits)
    /// Arousal: 0.0 to 1.0 (10 bits)
    /// Dominance: 0.0 to 1.0 (10 bits)
    /// Confidence: 0.0 to 1.0 (10 bits)
    pub fn new(valence: f32, arousal: f32, dominance: f32, confidence: f32) -> Self {
        let v_norm = ((valence.clamp(-1.0, 1.0) + 1.0) / 2.0 * 1023.0) as u64;
        let a_norm = (arousal.clamp(0.0, 1.0) * 1023.0) as u64;
        let d_norm = (dominance.clamp(0.0, 1.0) * 1023.0) as u64;
        let c_norm = (confidence.clamp(0.0, 1.0) * 1023.0) as u64;

        let packed = (v_norm) | (a_norm << 10) | (d_norm << 20) | (c_norm << 30);
        Self(packed)
    }

    /// Unpack to EmotionalVector (timestamp will be current block time)
    pub fn unpack_to_vector(&self) -> EmotionalVector {
        let v_raw = (self.0 & 0x3FF) as f32;
        let a_raw = ((self.0 >> 10) & 0x3FF) as f32;
        let d_raw = ((self.0 >> 20) & 0x3FF) as f32;
        // Confidence is at bits 30-39

        let valence = (v_raw / 1023.0 * 2.0) - 1.0;
        let arousal = a_raw / 1023.0;
        let dominance = d_raw / 1023.0;
    ...
    pub fn get_confidence(&self) -> f32 {
        let c_raw = ((self.0 >> 30) & 0x3FF) as f32;
        c_raw / 1023.0
    }
}
...
mod tests {
    ...
    #[test]
    fn test_compact_packing() {
        let valence = 0.5;
        let arousal = 0.8;
        let dominance = 0.2;
        let confidence = 0.9;
        
        let compact = CompactEmotionalState::new(valence, arousal, dominance, confidence);
        let unpacked = compact.unpack_to_vector();
        let unpacked_confidence = compact.get_confidence();
        
        // Check with small epsilon for float precision
        assert!((unpacked.valence - valence).abs() < 0.01);
        assert!((unpacked.arousal - arousal).abs() < 0.01);
        assert!((unpacked.dominance - dominance).abs() < 0.01);
        assert!((unpacked_confidence - confidence).abs() < 0.01);
    }
