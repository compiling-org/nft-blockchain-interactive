...
impl ShaderRenderer {
    ...
    pub fn update_audio_bands(&mut self, bands: Vec<f32>) {
        if bands.len() >= 4 {
            ...
            self.audio_bands = [bands[0], bands[1], bands[2], 0.0];
        }
    }
    ...
    fn update_standard_uniforms(&self) -> Result<(), JsValue> {
        ...
        // Update emotion uniform if present
        if let Some(emotion_loc_obj) = self.uniforms.get("u_emotion") {
            if let Some(emotion_loc) = emotion_loc_obj.as_ref() {
                self.gl.uniform3f(Some(emotion_loc), self.emotion[0], self.emotion[1], self.emotion[2]);
            }
        }
    ...
    pub fn set_emotion(&mut self, valence: f32, arousal: f32, dominance: f32) {
        self.emotion = [valence, arousal, dominance];
    }
    
    #[wasm_bindgen]
    pub fn get_current_preset(&self) -> String {
        self.current_preset.clone().unwrap_or_default()
    }
    
    #[wasm_bindgen]
    pub fn get_emotion(&self) -> Vec<f32> {
        vec![self.emotion[0], self.emotion[1], self.emotion[2]]
    }
    /// Get current time
    #[wasm_bindgen]
...
void main() {
    ...
    color = mix(color, vec3(0.5 + 0.5 * u_emotion.x, 0.5 + 0.5 * u_emotion.y, 0.5 + 0.5 * u_emotion.z), 0.25);
...
void main() {
    ...
    color = mix(color, vec3(0.5 + 0.5 * u_emotion.x, 0.5 + 0.5 * u_emotion.y, 0.5 + 0.5 * u_emotion.z), 0.2);
    gl_FragColor = vec4(color, 1.0);
}
"#;
