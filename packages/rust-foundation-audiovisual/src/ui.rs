...
fn audio_ui_system(
    ...
    // Only render UI if egui context is available and initialized
    let ctx = match contexts.ctx_mut() {
        Ok(ctx) => ctx,
        Err(_) => return,
    };

    // Check if egui is properly initialized by ensuring Context::run() has been called
    // This prevents the "Called available_rect() before Context::run()" panic
    if ctx.input(|i| i.time == 0.0) {
        // Skip UI rendering if egui context hasn't been initialized yet
        tracing::debug!("Skipping UI rendering - egui context not initialized (time == 0.0)");
        return;
    }

    tracing::debug!("Rendering audio UI system");
